    ...
    public partial class FormChonCTDDH : Form
    {
        ...
        private void nbrSoLuong_Validating(object sender, CancelEventArgs e)
        {
            if (nbrSoLuong.Value > soLuongMax)
            {
                e.Cancel = true;
                nbrSoLuong.Focus();
                errorProvider1.SetError(nbrSoLuong, "Số lượng không được quá số lượng trong chi tiết đơn hàng !");
            }
            ...
            if (nbrSoLuong.Value == 0)
            {
                e.Cancel = true;
                nbrSoLuong.Focus();
                errorProvider1.SetError(nbrSoLuong, "Vui lòng chọn số lượng !");
            }
            else
            {
                e.Cancel = false;
                errorProvider1.SetError(nbrSoLuong, "");
            }
        }
        ...
        private void nbrDonGia_Validating(object sender, CancelEventArgs e)
        {
            if (nbrDonGia.Value < 1)
            {
                e.Cancel = true;
                nbrSoLuong.Focus();
                errorProvider1.SetError(nbrDonGia, "Đơn giá phải lớn hơn 0 !");
            }
            else
            {
                e.Cancel = false;
                errorProvider1.SetError(nbrDonGia, "");
            }
        }
        ...
        private void txtmaDDH_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txtmaDDH.Text))
            {
                e.Cancel = true;
                txtmaDDH.Focus();
                errorProvider1.SetError(txtmaDDH, "vui lòng chọn chi tiết đơn hàng !");
            }
            else
            {
                e.Cancel = false;
        ...
        private void txtTenVatTu_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txtTenVatTu.Text))
            {
                e.Cancel = true;
                txtTenVatTu.Focus();
                errorProvider1.SetError(txtTenVatTu, "vui lòng chọn chi tiết đơn hàng !");
            }
            else
            {
                e.Cancel = false;
                errorProvider1.SetError(txtTenVatTu, "");
            }
        }
        ...
        private void txtMaVT_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                errorProvider1.SetError(txtMaVT, "chưa chọn chi tiết đơn hàng !");
            }
            if (txtMaVT.Text.Trim().Contains(" "))
            {
                e.Cancel = true;

                errorProvider1.SetError(txtMaVT, "vui lòng chọn chi tiết đơn hàng !");
            }
